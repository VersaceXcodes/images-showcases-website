...
const UV_SingleImageView: React.FC = () => {
  ...
  // Fetch image details
  const { data: imageDetails, isPending: isImageLoading, error: imageError } = useQuery<Image, Error>({
      ...
      <div className="container mx-auto p-4">
        {imageDetails && (
          <div className="flex flex-col items-center">
            <img src={imageDetails.image_url} alt={imageDetails.title} className="max-w-full h-auto" />
            ...
            <div className="mt-2">
              {imageDetails.categories && (
                <span className="bg-gray-200 rounded-full text-sm px-2 py-1 m-1">{imageDetails.categories}</span>
              )}
            </div>
            <button
